...
const sections: SectionConfig<PatientProfileResponse>[] = [
  {
    ...
    fields: [
      ...
      },
      {
        ...
        accessor: (data) => data.preferredBranch?.name ?? "Not selected",
      },
      {
        label: "Gender",
        accessor: "gender",
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {data.gender ? renderBadge(data.gender) : null}
            {data.preferredBranch?.name ? (
              <Badge variant="outline">{data.preferredBranch.name}</Badge>
            ) : null}
          </div>
        </div>
      </div>
