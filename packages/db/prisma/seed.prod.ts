...
async function seedCommerce(
  ...
  const supplementsCategory = await prisma.productCategory.create({
    data: {
      name: "Supplements",
      slug: "supplements",
      description: "Vitamins and supplements for everyday health.",
      isActive: true,
    },
  });
  ...
  const skincareCategory = await prisma.productCategory.create({
    data: {
      name: "Skincare",
      slug: "skincare",
      description: "Skincare essentials and dermatology-friendly products.",
      isActive: true,
    },
  });
  ...
  const vitaminImage = await prisma.media.create({
    data: {
      uploadedById: foundation.admin.id,
      publicId: "prod-seed/product-vitamin-d",
      url: `${DOCTOR_AVATAR_URLS[1]}?v=vitamin-d`,
      mimeType: "image/png",
      resourceType: "image",
      size: 110000,
      hash: "prod-seed-product-vitamin-d-hash",
      name: "vitamin-d.png",
      type: MediaType.product,
      visibility: MediaVisibility.public,
      altText: "Vitamin D bottle",
    },
  });
  ...
  const creamImage = await prisma.media.create({
    data: {
      uploadedById: foundation.admin.id,
      publicId: "prod-seed/product-repair-cream",
      url: `${DOCTOR_AVATAR_URLS[2]}?v=repair-cream`,
      mimeType: "image/png",
      resourceType: "image",
      size: 115000,
      hash: "prod-seed-product-repair-cream-hash",
      name: "repair-cream.png",
      type: MediaType.product,
      visibility: MediaVisibility.public,
      altText: "Repair cream",
    },
  });

  const vitaminD = await prisma.product.create({
    data: {
      categoryId: supplementsCategory.id,
      imageId: vitaminImage.id,
      name: "Vitamin D3 2000 IU",
      slug: "vitamin-d3-2000iu",
      sku: "SKU-VITD3-2000",
      description: "Daily Vitamin D3 support (demo product).",
      price: 14.99,
      salePrice: 12.99,
      stockQuantity: 250,
      isActive: true,
  ...
  const repairCream = await prisma.product.create({
    data: {
      ...
      imageId: creamImage.id,
      name: "Barrier Repair Cream",
      slug: "barrier-repair-cream",
      sku: "SKU-SKIN-REPAIR-01",
      description: "Hydrating repair cream for sensitive skin (demo product).",
      price: 18.5,
      salePrice: 16.0,
      stockQuantity: 120,
      isActive: true,
  ...
  const wellnessTea = await prisma.product.create({
    data: {
      categoryId: wellnessCategory.id,
      name: "Wellness Herbal Tea",
      slug: "wellness-herbal-tea",
      sku: "SKU-WELL-TEA-01",
      description: "Caffeine-free herbal tea (demo product).",
      price: 9.5,
      stockQuantity: 300,
      isActive: true,
    },
...
async function main() {
  ...
  const commerce = await seedCommerce(foundation, profiles);
